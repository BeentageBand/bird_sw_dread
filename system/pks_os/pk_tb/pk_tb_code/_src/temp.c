 ...
 *=====================================================================================*/
 ...
 {
    TB_OS tb_os = TB_get_instance(0,0);
    Isnt_Nullptr(tb_os, 0);
    return TB_OS_search_task(tb_os);
 }
 ...
 TB_Process_Id_T TB_self_pid(void)
 {
    TB_OS tb_os = TB_get_instance(0,0);
    Isnt_Nullptr(tb_os, 0);
    return TB_OS_search_pid(tb_os);
 }

 void TB_task_ready(void)
 {
    TB_OS tb_os = TB_get_instance(0,0);
    TB_OS_notify_ready(tb_os, TB_self_task_id());
 }

 void TB_create_mailbox(uint32_t const max_mails, size_t const mail_size)
 {
    TB_OS tb_os = TB_get_instance(0,0);
    TB_set_mailbox(tb_os, max_mails, mail_size);
 }
 ...
 */
 ...
 {
    TB_OS tb_os = TB_get_instance(0,0);
    TB_Mailbox mailbox = TB_OS_search_mailbox(tb_os, receiver_task, receiver_pid);
    TB_Mail mail = TB_Mail_New(mail_id, TB_OS_search_task(tb_os), receiver_task, receiver_pid, data, data_size);
 ...
 void TB_broadcast(TB_Mail_Id_T const mail_id, void const * data, size_t const data_size)
 {
    ...
    for(process = 0 ; process < MAX_PROCESSES; ++process)
    {
       ...
       {
          mailbox = TB_OS_search_mailbox(tb_os, task, process);
          if(NULL != mailbox)
          {
             mail = TB_Mail_New(mail_id, TB_OS_search_task(tb_os), task, process, data, data_size);
             TB_Mailbox_push_mail(mailbox, mail);
             TB_Mail_delete(mail);
          }
       }
      ...
      uint32_t const timeout_ms)
 {
    ...
    TB_OS tb_os = TB_get_instance(0,0);
    TB_Mail mail = NULL;
    TB_Mailbox mailbox = TB_OS_search_mailbox(tb_os, TB_self_task_id(), TB_self_pid());
