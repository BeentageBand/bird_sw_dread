...
void data_collector_wrkr_on_mail(union Worker * const super, union Mail * const mail)
{
  ...
  DCFSM->State_Machine.vtbl->dispatch(DCFSM, mail);
}
...
union Data_Collector_Wrkr * Allocate_Data_Collector_Wrkr(void)
{
  if(NULL == Data_Collector_Wrkr.vtbl)
  {
     ...
     Data_Collector_Wrkr_Class.Worker.on_start = data_collector_wrkr_on_start;
     Data_Collector_Wrkr_Class.Worker.on_loop = data_collector_wrkr_on_loop;
     Data_Collector_Wrkr_Class.Worker.on_mail = data_collector_wrkr_on_mail;
     Data_Collector_Wrkr_Class.Worker.on_stop = data_collector_wrkr_on_stop;
